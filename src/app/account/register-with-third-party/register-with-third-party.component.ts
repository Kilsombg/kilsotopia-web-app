...
export class RegisterWithThirdPartyComponent implements OnInit {
        ...
        if (user) {
          ...
          this.activatedRoute.queryParamMap.subscribe({
            next: (params: any) => {
              ...
              if (this.provider && this.access_token && this.userId && this.provider === 'google') {
                this.initializeForm();
              } else {
  ...
  register() {
    ...
    if (this.registerForm.valid && this.userId && this.access_token && this.provider) {
      ...
      const model = new RegisterWithExternal(firstName, this.userId, this.access_token, this.provider);
